...
const candidates = ['Ravi', 'Kate', 'Robin'];
...
.then((accounts) => {
  ...
  .then((result) => {
    ...
    return deployedContract.methods.totalVotesFor(asciiToHex('Ravi')).call();
  })
  .then((votesRavi) => {
    console.log('votesRavi', votesRavi);
    return deployedContract.methods.voteForCandidate(asciiToHex('Ravi')).send();
  })
  .then((voteResult) => {
